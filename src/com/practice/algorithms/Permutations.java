...
public class Permutations {                               
 ...
 private static void permute(String s, String chosen) {                                     
     ...
     if (s.length() == 0) {                                 
         ...
         for (int i = 0; i < s.length(); i++) {                                               
             ...
             // un-choose                                                      
             chosen = chosen.substring(0, chosen.length() - 1);      //coz you'd always add only one letter
             //So remove that last letter added.
 ...
 /*
 private static void permute(String s, String chosen) {
     Recursion.println("permute " + s + ", " + chosen);
     if (s.length() == 0) {
         System.out.println(chosen);  // base case
     } else {
         for (int i = 0; i < s.length(); i++) {
             // choose
             String chosenLetter = s.substring(i, i+1);
             String newS = s.substring(0, i) + s.substring(i+1, s.length());        // remove firstLetter from s
